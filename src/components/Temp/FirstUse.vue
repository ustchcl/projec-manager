    ...
    <v-dialog v-model="dialog" width="600" persistent>
      <v-card>
        <v-card-title
          ...
          dark
          primary-title
          v-html="$lang.Get('firstUseWelcome')"
        >
        </v-card-title>

        <v-card-text class="text-xs-center" v-html="$lang.Get('firstUseContent')">
        </v-card-text>

        <v-divider class="py-4"></v-divider>
        <v-card-title>
          <p class="ma-0 ml-4">{{ $lang.Get("firstUseSetupFolder") }}</p>
          <FolderInput v-model="defaultFolder"></FolderInput>
        </v-card-title>
        <v-card-title class="pa-0">
          <v-card-text class="ma-0 ml-4">{{ $lang.Get("firstUseSetupColor") }}</v-card-text>
